...
=> B contract에서 A contract의 함수 또는 변수 a를 호출하는데, 만약 A contract에 a가 없다면, 애러가 발생하지만, fallback함수를 넣어둔다면, 애러대신 fallback 함수가 호출될 것
...
contract You{
    ...
    function DepositWithSend(address payable _to) public payable{
         bool success = _to.send(msg.value); //2300gas
